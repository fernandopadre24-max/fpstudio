...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const verifyStudioPin = useCallback(
    (pinToTest: string, staffMember: StudioStaffUser) => {
      ...
      const currentPin = adminCredentials?.pin || staffMember.pins[0] || '0000';
      const backupPins = adminCredentials?.backupPins || [];
      const allowedPins = [currentPin, ...backupPins].filter(Boolean);
  ...
  const handleStudioEmailLogin = (e?: React.FormEvent) => {
    ...
    const isPassValid = admPassword === expectedPass;
